...
import { Pencil, Trash2, Plus, ImageOff, Star, Search } from "lucide-react";
...
function Dashboard() {
  ...
  const onToggleSoldOut = async (p: Product) => {
    try {
      await updateProduct(p.id, { isSoldOut: !p.isSoldOut });
      toast.success(p.isSoldOut ? "Marked as Available" : "Marked as Sold Out");
      reload();
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Update failed");
    }
              ...
              <table className="w-full text-sm">
                <thead className="bg-ivory/[0.04] text-ivory/60 uppercase tracking-widest text-xs">
                  <tr>
                    ...
                    <th className="text-center p-4 hidden sm:table-cell">Sold Out</th>
                    ...
                    <tr key={p.id} className="border-t border-ivory/10 hover:bg-ivory/[0.03]">
                      ...
                      <td className="p-4 hidden sm:table-cell text-center">
                        <button
                          ...
                          className={`p-2 rounded-md transition ${p.isFeatured
                      ...
                      <td className="p-4 hidden sm:table-cell text-center">
                        <button
                          ...
                          title={p.isSoldOut ? "Mark as Available" : "Mark as Sold Out"}
                          className={`p-2 rounded-md transition ${p.isSoldOut
                            ? "text-destructive hover:bg-destructive/10"
                            : "text-ivory/30 hover:text-ivory/60 hover:bg-ivory/10"
                            }`}
                        >
                          <div className={`w-4 h-4 rounded-full border-2 ${p.isSoldOut ? "bg-destructive border-destructive" : "border-ivory/30"}`} />
                        </button>
                      </td>
                      <td className="p-4">
